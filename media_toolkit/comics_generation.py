...
def get_user_input():
    ...
    return instructions, student_class, num_panels

def create_comical_story_prompt(instructions, student_class, num_panels):
    """
    Uses GPT-4o to turn the user's instructions into a series of detailed prompts for DALL-E 3,
    based on the specified number of panels and target audience.
    """
...
def generate_comic_image(prompt, panel_number):
    """
    Uses DALL-E 3 to generate a comic book panel image based on the prompt.
    ...
    try:
        response = client.images.generate(
            model="dall-e-3",
            prompt=f"A colorful and fun children's comic book style illustration for a multi-panel comic strip. The panel shows: {prompt}",
            size="1024x1024",
            quality="standard",
            n=1,
        )
...
def main():
    ...
    """
    instructions, student_class, num_panels = get_user_input()
    if not instructions:
        print("No instructions provided. Exiting.")
        return

    comical_story_prompts = create_comical_story_prompt(instructions, student_class, num_panels)
    ...
    for i, prompt in enumerate(panel_prompts):
        ...
        image_url = generate_comic_image(prompt, panel_number)
        
        if image_url:
            print(f"Comic Panel URL: {image_url}")
        else:
