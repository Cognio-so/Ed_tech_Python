...
class ImageGenerator:
    ...
    DALL-E 3 model, with prompts enhanced by GPT-4o.
    ...
    def _rephrase_schema_to_prompt(self, schema: dict) -> str:
        ...
        for the DALL-E 3 image generation model using GPT-4o.
        ...
        """
        required_keys = ["topic", "preferred_visual_type", "instructions", "subject", "grade_level"]
        for key in required_keys:
            if key not in schema:
                raise KeyError(f"The schema is missing the required key: '{key}'")

        # Construct a detailed meta-prompt for GPT-4o to generate a DALL-E 3 prompt.
        ...
        gpt4o_prompt = (
            ...
            "The output must use DALL·E 3’s strengths in structured visual composition and label clarity.\n\n"
            ...
            "- if subject is specified show the labels, it should be **concise and accurately spelled** without ant distortions in lable\n"
            ...
            "Based on this schema, generate a prompt for DALL·E 3. The prompt must:\n"
            ...
            f"5. If the schema includes a difficulty flag set to {schema['difficulty_flag']}, increase the level of anatomical accuracy, shading, and depth.\n"
            f"6. Explicitly list all labels that must appear for: {schema['topic']} if Subject is specified.\n\n"

            "Final Output Prompt for DALL·E 3 should be natural, instructional, and not include markdown formatting."
        )
        ...
        try:
            ...
            return f"A high-quality {schema['preferred_visual_type']} about {schema['topic']} for a {schema['subject']} lesson at grade {schema['grade_level']}. Instructions: {schema['instructions']}"
    ...
    def generate_image_from_schema(self, schema: dict) -> str:
        """
        Generates an image using the DALL-E 3 model based on the provided schema,
        ...
        try:
            ...
            prompt = self._rephrase_schema_to_prompt(schema)
            
            if not prompt:
                print("Prompt generation failed.")
                return None

            print("Requesting image from OpenAI DALL-E 3 API...")
            
            # The API call to generate an image using dall-e-3
...
if __name__ == "__main__":
    ...
    # For Linux/macOS: export OPENAI_API_KEY='your_key_here'
    ...
    try:
        ...
        # Generate the image
        generated_image_url = generator.generate_image_from_schema(image_schema)

        if generated_image_url:
            print("\nTo view the generated image, open the following URL in your browser:")
